...
為了讓RAG系統能夠更精確地理解和檢索您的Notion內容，建議按照以下方式組織您的文件：
...
```
✅ 好的寫法：
API（Application Programming Interface）是應用程式之間溝通的介面，
允許不同軟體元件互相交換資料和功能。

❌ 避免的寫法：
API 就是那個東西，用來連接不同的系統。
```
...
✅ 完整步驟：
1. 開啟終端機或命令提示字元
2. 輸入 `pip install requests` 安裝套件
3. 確認安裝成功：`pip show requests`
4. 在 Python 程式中匯入：`import requests`

❌ 不完整說明：
安裝 requests 套件然後就可以用了。
```
...
Q: 系統支援哪些瀏覽器？
A: 支援 Chrome 80+、Firefox 75+、Safari 13+、Edge 80+
```

#### 2. **流程圖和步驟**
```
🔄 部署流程：
開發完成 → 程式碼審查 → 測試環境部署 → 測試驗證 → 正式環境部署 → 監控確認
...
| 項目 | 說明 | 範例 | 注意事項 |
|------|------|------|----------|
| 用戶ID | 唯一識別碼 | USR001 | 不可重複 |
| 用戶名稱 | 顯示名稱 | 張小明 | 2-20字元 |
| 電子郵件 | 聯絡方式 | user@example.com | 必須有效 |
```
...
#### 1. **使用同義詞和別名**
```
登入 / 登錄 / Sign In / Authentication
使用者 / 用戶 / User
設定 / 設置 / Configuration / Settings
```
...
在頁面開頭添加：
```
📋 摘要：本頁面說明系統登入功能的設計與實作
🏷️ 關鍵字：登入、身份驗證、安全性、用戶管理
📅 最後更新：2024-01-20
```
...
```
📝 更新記錄：
v2.1 (2024-01-20)：新增LINE Bot功能說明
v2.0 (2024-01-15)：重新整理系統架構
v1.5 (2024-01-10)：修正API文件錯誤
```
...
2. 登入後進入 Dashboard
3. 在「Your Authtoken」頁面複製您的 authtoken
...
# Windows
# 1. 從 https://ngrok.com/download 下載
# 2. 解壓縮到適當位置
# 3. 將 ngrok.exe 路徑加入系統環境變數
...
# 複製 https://abc123.ngrok-free.app 這個 URL
```

**步驟5：設定 ngrok 免費版限制處理**
```bash
# 免費版每次重啟會更換 URL，建議設定固定子域名（付費功能）
# 或使用以下方式獲得相對穩定的連線：
ngrok http 5000 --region=ap  # 使用亞太區域節點
```
